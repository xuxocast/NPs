...
//   a very special partition set to fast convergence (See DPGK_Total for the 
//   brute-fore counterpart). The results are written to a file named "DP*.dat". 
...
const int Nt = 15 ;       // Must be odd integer!!! Integrates 3n+1 polynomials
const int Nf = 19;  
...
sprintf(filename, "DP_a%.2g_v%.2g_b%.2gnm.dat", a/(1.*nm), vv, b/(1.*nm));
FILE *fpp = fopen(filename,"w+");
fprintf(fpp,"Total momentum transfered, a: %.2gnm    v: %.2gc   b: %.2gnm   \n", a/(1.*nm), vv, b/(1.*nm));
fprintf(fpp,"\n");
fprintf(fpp,"l           DPEx                    DPHx                  DPEz                  DPHz            DPEsx                   DPHsx                 DPEsz                 DPHsz\n");


char filenamer[sizeof "DP_a1nm_v0.99c_b1.5nm_error.dat"];
sprintf(filenamer, "DP_a%.2g_v%.2g_b%.2gnm_error.dat", a/(1.*nm), vv, b/(1.*nm));
FILE *fppe = fopen(filenamer,"w+");
fprintf(fppe,"Total momentum transfered, a: %.2gnm    v: %.2gc   b: %.2gnm   \n", a/(1.*nm), vv, b/(1.*nm));
fprintf(fppe,"\n");
fprintf(fppe,"l      errDPEx                   errDPHx                 errDPEz                errDPHz        errDPEsx               errDPHsx               errDPEsz              errDPHsz\n");


char filenamex[sizeof "dpdw_a1nm_v0.99c_b1.5nm.dat"];
sprintf(filenamex, "dpdw_a1nm_v%.2g_b%.2gnm.dat", vv, b/(1.*nm));
FILE *fpx = fopen(filenamex,"w+");
fprintf(fpx,"l          w(au)                   dpEdwx                 dpHdwx                 dpEdwz                dpHdwz                  dpEsdwx                dpHsdwx                dpEsdwz                dpHsdwz\n");
...
// Calculates  common parameters of the scatterred fields
    for (int m = -l; m <= l; ++m){ 
        DE[l-1][m + l] = alm(l,m)*dle*PsiE(l,m,w,b,vv);
        CM[l-1][m + l] = alm(l,m)*clm*PsiM(l,m,w,b,vv);
    }
...
DPx[0] += (xk[i]*(1. + 1i) - 1i*xg[i])*dpdwx[0].real();
DPz[0] += (xk[i]*(1. + 1i) - 1i*xg[i])*dpdwz[0].real();
DPx[1] += (xk[i]*(1. + 1i) - 1i*xg[i])*dpdwx[1].real();
DPz[1] += (xk[i]*(1. + 1i) - 1i*xg[i])*dpdwz[1].real();         // Integrates in frequency

DPx[2] += (xk[i]*(1. + 1i) - 1i*xg[i])*dpdwx[2].real();
DPz[2] += (xk[i]*(1. + 1i) - 1i*xg[i])*dpdwz[2].real();
DPx[3] += (xk[i]*(1. + 1i) - 1i*xg[i])*dpdwx[3].real();
DPz[3] += (xk[i]*(1. + 1i) - 1i*xg[i])*dpdwz[3].real();


// Here print the dpdw's, for each, w, l
fprintf(fpx,"%d %.17g %.17g %.17g %.17g %.17g %.17g %.17g %.17g %.17g \n",l1,w,dpdwx[0].real(),dpdwx[1].real(),dpdwz[0].real(),dpdwz[1].real(), dpdwx[2].real(), dpdwx[3].real(), dpdwz[2].real(), dpdwz[3].real());

}// for w

// Here print the total momentum
fprintf(fpp,"%d %.17g %.17g %.17g %.17g  %.17g %.17g %.17g %.17g \n",l1,DPx[0].real(),DPx[1].real(),DPz[0].real(),DPz[1].real(), DPx[2].real(),DPx[3].real(),DPz[2].real(),DPz[3].real());
fprintf(fppe,"%d %.17g %.17g %.17g %.17g %.17g %.17g %.17g %.17g \n",l1,DPx[0].imag(),DPx[1].imag(),DPz[0].imag(),DPz[1].imag(), DPx[2].imag(),DPx[3].imag(),DPz[2].imag(),DPz[3].imag());
